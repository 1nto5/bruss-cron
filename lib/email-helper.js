 ...
 * @param {{ displayName?: string, email: string, hours: number }[]} usersData
...
export function buildSummaryTable(usersData) {
  const rows = usersData
    .map(
      (u) =>
        `<tr><td style="padding:4px 8px;border:1px solid #ddd;">${escapeHtml(u.displayName || u.email)}</td><td style="padding:4px 8px;border:1px solid #ddd;text-align:right;">${u.hours}h</td></tr>`
    )
    .join('');
  return `<table style="border-collapse:collapse;margin:10px 0;"><thead><tr><th style="padding:4px 8px;border:1px solid #ddd;text-align:left;">Employee</th><th style="padding:4px 8px;border:1px solid #ddd;">Hours</th></tr></thead><tbody>${rows}</tbody></table>`;
}
