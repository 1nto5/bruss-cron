...
/**
 * Generic callback-to-Promise wrapper for SMB2 client methods.
...
export async function listFiles(client, path = '') {
  const files = await smbCall(client, 'readdir', path);
  return files
...
export async function fileExists(client, filePath) {
  try {
    return await smbCall(client, 'exists', filePath);
  } catch {
    return false;
  }
}
...
export async function getFileStats(client, filePath) {
  try {
    const stats = await smbCall(client, 'stat', filePath);
...
export async function isDirectory(client, path) {
  try {
    await smbCall(client, 'readdir', path);
    return true;
  } catch {
    return false;
  }
