 ...
 */
export async function monitorEOL308Backup() {
  ...
  try {
    ...
    const monitorPath = process.env.SMB_EOL308_MONITOR_PATH;
    ...
    if (!monitorPath) {
      throw new Error(
        'Missing EOL308 monitoring configuration in environment variables'
      );
    }
    ...
    if (statusJson.exitCode !== 0) {
      const errorMsg =
        statusJson.errors && statusJson.errors.length > 0
          ...
          : 'Unknown error';
      throw new Error(
        `Last backup failed with exit code ${statusJson.exitCode}: ${errorMsg}`
      );
    }
    ...
    const result = {
      ...
      formattedSize: statusJson.totalSize || 'N/A',
    ...
    console.error('Error in monitorEOL308Backup:', error);
    throw error;
  }
}
